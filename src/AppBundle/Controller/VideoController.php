	...
	public function newAction(Request $request)
	{
		...
		if ($authCheck == true) {
			...
			if($json !== null){
				...
				$userId = (isset($identity->sub) && $identity->sub != null) ? $identity->sub : null;
		...
		if ($authCheck == true) {
			...
			if($json !== null){
				...
				if($userId != null && $title != null){
					...
					if(isset($identity->sub) && is_object($video) && $identity->sub == $video->getUser()->getId()){
						$video->setTitle($title);
						$video->setDescription($description);
						$video->setStatus($status);
						$video->setUpdatedAt($updatedAt);
						...
						$data = array(
							"status" => "error",
							"code" => 200,
							"data" => "Upload Video failed, you aren't owner!!"
						);
					}
				}else {
					$data = array(
			...
			if(isset($identity->sub) && is_object($video) && $identity->sub == $video->getUser()->getId()){
				...
				if ($file != null && !empty($file)){
					...
					if($ext == "jpeg" || $ext == "jpg" || $ext == "png" || $ext == "gif"){
						$fileName = time() . "." . $ext;
						$pathOfFile = "uploads/video_images/video_" . $id;
						...
						$data = array(
							"status" => "error",
							"code" => 400,
							"data" => "Format for image not valid!!"
						);
					}
					...
					if($fileVideo != null && !empty($fileVideo)){
						...
						if($ext == "mp4" || $ext == "avi"){
							$fileName = time() . "." . $ext;
							$pathOfFile = "uploads/video_files/video_" . $id;
							...
							$data = array(
								"status" => "error",
								"code" => 400,
								"data" => "Format for video not valid!!"
							);
						}
					}else{
						$data = array(
